...
app.post("/api/master/v2/customers/provision", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    await admin.auth().revokeRefreshTokens(user.uid);
    try { await pool.query(`insert into app_users (uid, email, company, role, active) values ($1,$2,$3,$4,true) on conflict (uid) do update set active=true, company=$3, role=$4`, [user.uid, email, company, "admin"]); } catch {}

    return res.json({ ok:true, uid:user.uid, email, password });
  } catch (e) {
    console.error("POST /api/master/v2/customers/provision", e);
    return res.status(500).json({ error:"server_error" });
  }
});
...
app.post("/api/master/v2/customers/delete", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    const user = await getUserByUidOrEmail({ uid, email });
    await admin.auth().revokeRefreshTokens(user.uid);   // ★ 既存セッション無効化
    ...
    console.error("POST /api/master/v2/customers/delete", e);
    return res.status(500).json({ error:"server_error" });
  }
});
...
app.post("/api/master/v2/customers/fix-claims", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    await admin.auth().revokeRefreshTokens(user.uid);   // ★ 再ログインで確実反映
    try { await pool.query(`update app_users set company=$1, role=$2 where uid=$3`, [company, role, user.uid]); } catch {}

    return res.json({ ok:true, uid:user.uid, email, claims });
  } catch (e) {
    console.error("POST /api/master/v2/customers/fix-claims", e);
    return res.status(500).json({ error:"server_error" });
  }
});
...
app.get("/api/debug/v2/user-by-email", async (req, res) => {
  try {
    const email = String(req.query.email||"").trim();
    const user = await getUserByUidOrEmail({ email });
    return res.json({ ok:true, uid:user.uid, email:user.email, customClaims:user.customClaims||null });
  } catch (e) {
    return res.status(404).json({ ok:false, error:e?.code||"not_found" });
  }
});
